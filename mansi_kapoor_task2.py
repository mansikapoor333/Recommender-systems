...
def get_user_rating(x, keyed_ratings, list_userwise_businesses, businesswise_users, N):
    ...
    ctr1 = 0
    avg_rating1 = float(keyed_ratings[(user_id1, list_userwise_businesses[user_id1][0])][1])
    ...
    if (business_id1 not in businesswise_users.keys()):
        ...
        for i in co_rated_user_ids:
            ...
            for j in co_rated_businesses:
                if (j != business_id1 and ctr < N):
                    ...
                    pass
            if (numerator != 0):
                cosine_similarity = float(numerator) / float(math.sqrt(denominator1 * denominator2))
        ...
        for i in cosine_similarities:
            if (count < N):
                ...
                co_rating = float(keyed_ratings[(co_rater, business_id1)][0]) - float(avg_rating)

                num = num + float(co_rating * cosine_similarity)
        ...
        if (den == 0):
            ...
            rating = float(avg_rating1) + (float(num) / float(den))
